            ...
            [:div
             ...
             [:div.mulligan
              (if (or (= :spectator @my-side)
                ...
                (list ^{:key "keepbtn"}
                      [cond-button (tr [:game.keep "Keep"])
                       (= "mulligan" (:prompt-type @promp-state))
                       #(send-command "choice" {:choice {:uuid (->> (:choices @promp-state)
                                                                    (filter (fn [c] (= "Keep" (:value c))))
                                                                    first
                                                                    :uuid)}})]
                      ^{:key "mullbtn"}
                      [cond-button (tr [:game.mulligan "Mulligan"])
                       (= "mulligan" (:prompt-type @promp-state))
                       #(do (send-command "choice" {:choice {:uuid (->> (:choices @promp-state)
  ...
  [:div.panel.blue-shade
   ...
   (cond
     ...
     (let [counter-type (keyword (:counter choices))
           num-counters (get-in card [:counter counter-type] 0)]
       [:div
        [:div.credit-select
         [:select#credit
  ...
  (let [autocomp (r/track (fn [] (get-in @prompt-state [:choices :autocomplete])))
    ...
    (r/create-class
      {:display-name "button-pane"
       ...
       :reagent-render
       (fn [{:keys [side active-player run end-turn runner-phase-12 corp-phase-12 corp runner me opponent prompt-state]
             :as button-pane-args}]
